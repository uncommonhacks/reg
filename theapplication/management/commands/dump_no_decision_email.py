...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("outfile", help="file to dump json output to")

    def handle(self, *args, **options):
        db_data = [get_data_from_applicant(a) for a in Applicant.objects.filter(~Q(application=None)).filter(notified_of_admit_status=False).all()]

        with open(
            str(Path.home()) + "/" + options["outfile"], "w", encoding="utf-8"
        ) as f:
