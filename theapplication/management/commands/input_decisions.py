...
def do_status_routine(username_file, decision, mystdout):
    ...
    for uname in usernames:
        ...
        if applicant.status == "NS":
            skip = get_skip_confirmation(
                "Applicant has not finished application. Are you sure you want to make a decision?"
            )
        elif applicant.status == "CF":
            skip = get_skip_confirmation(
                "Applicant is confirmed already. Are you sure you want to change their status?"
            )
        elif applicant.status == new_status and applicant.notified_of_admit_status:
            skip = get_skip_confirmation(
                "Applicant already has this status and has been notified. Mark for re-notification?"
            )
        if skip:
            continue
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--accept_file",
            nargs="?",
            help="filename of newline-separated usernames for accounts to accept",
        )
        parser.add_argument(
            "--waitlist_file",
            nargs="?",
            help="filename of newline-separated usernames for accounts to waitlist",
        )
        parser.add_argument(
            "--reject_file",
            nargs="?",
            help="filename of newline-separated usernames for accounts to reject",
        )
