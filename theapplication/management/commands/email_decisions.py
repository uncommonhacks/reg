...
def send_notification_email(email_addr, first_name):
    context = {
        'site': settings.MAIN_URL,
        'first_name': first_name,
    }
    subject = render_to_string(
        template_name='emails/decision_email_subject.txt',
        context=context
    )
    subject = ''.join(subject.splitlines())
    message = render_to_string(
        template_name='emails/decision_email_contents.html',
        context=context
    )
    plain_message = strip_tags(message)
    from_email = settings.DEFAULT_FROM_EMAIL
    mail.send_mail(subject, plain_message, from_email, [email_addr], html_message=message)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--force-username-file', nargs='?', help='instead of emailing everyone with a decision who hasn\'t been notified, email everyone on the list with their current status.')
